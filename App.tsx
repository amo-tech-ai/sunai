...
import { Screen, DeckData, Deck, Slide, TemplateID, Event, Perk } from './types';
...
const App: React.FC = () => {
  ...
  const renderScreen = () => {
    const dashboardScreens = [Screen.Dashboard, Screen.Profile, Screen.Events, Screen.EventDetail, Screen.MyEvents, Screen.Perks];
    if (dashboardScreens.includes(currentScreen)) {
        return (
            <DashboardLayout currentScreen={currentScreen} setCurrentScreen={setCurrentScreen}>
                ...
                {currentScreen === Screen.Perks && <PerksScreen perks={perksData} />}
            </DashboardLayout>
        );
    }
